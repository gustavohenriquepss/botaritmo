      ...
      <div className="w-full max-w-md space-y-8">
        <div>
          <h2 className="text-4xl font-normal text-[#1A1A1A] tracking-[-0.02em] font-display">
            {isForgot ? 'Recuperar Senha' : isLogin ? 'Sign In' : 'Sign Up'}
          </h2>
        ...
        <form onSubmit={handleSubmit} className="space-y-6">
          ...
          >
            {loading ? 'Loading...' : isForgot ? 'Enviar Link' : isLogin ? 'Sign In' : 'Sign Up'}
          </Button>
        </form>
        <div className="space-y-2">
          {!isForgot && (
            <button
              ...
              className="block w-full text-sm text-[#1A1A1A] hover:opacity-70 transition-opacity text-left"
            >
              Esqueceu sua senha?
            </button>
          )}
